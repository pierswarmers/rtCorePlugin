...
<li id="gnAttachedAsset<?php echo $asset->getId() ?>" class="gn-core-upload-item <?php echo ($asset->isImage()) ? 'thumbnail' : 'other' ?>">
  ...
  <span class="insert" onclick="injectTextIntoCurrent('<?php echo $asset->isImage() ? '!' : '' ?>[<?php echo $description_text; ?>](asset:<?php echo $asset->getOriginalFilename() ?><?php echo $asset->isImage() ? '|right|200,300' : '' ?>)')">&nbsp;</span>
</li>