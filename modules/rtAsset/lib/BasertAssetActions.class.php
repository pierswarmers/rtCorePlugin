...
class BasertAssetActions extends sfActions
{
    ...
    {
        if(!$request->hasParameter('id') || !$request->hasParameter('class')) {
            throw new sfException('Must have both class and id sent for listing.');
        }

        $this->forward404Unless($this->object = Doctrine::getTable($request->getParameter('class'))->findOneById($request->getParameter('id')));
    }
    ...
    public function executeUpdate(sfWebRequest $request)
    {
        ...
        $asset->setTargetUrl($request->getParameter('target_url'));
    ...
    public function executeUpload(sfWebRequest $request)
    {
        ...
        if ($form->isValid()) {
            try {
                ...
                $assets = Doctrine::getTable('rtAsset')->getAssetsForModelAndId(
                    $request_params['model'],
                    $request_params['model_id']
                );
