...
<script type="text/javascript">
  $(function() {
    $('#rt_site_<?php echo $form->getName()?>_country').change(function() {

      var holder =  $('#rt_site_<?php echo $form->getName()?>_state').parent();

      holder.html('<span class="loading">Loading states...</span>');
      $('#rt_site_<?php echo $form->getName()?>_state').remove();
      $.ajax({
        type: "POST",
        url: '<?php echo url_for('rtAdmin/stateInput') ?>',
        data: ({
          country : $(this).find('option:selected').attr('value'),
          id      : 'rt_site_<?php echo $form->getName()?>_state',
          name    : 'rt_site[<?php echo $form->getName()?>][state]'
        }),
        dataType: "html",
